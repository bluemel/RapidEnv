...
/**
 * the task to generate the RapidEnv Reference Configuration Language
 * Documentation in XML format usable for further transformation.
 * 
...
public final class TaskGenConfigLangInfo extends Task {
	 ...
	 * 
	 * @param argForce determines if the generation should be performed not
	 *                 regarding modification dates
	 */
	...
	/**
	 * Determine the input model root directory (folder). The model files are only
	 * used for modification date check.
	 * 
	...
	public void execute() {
		...
		if (this.type == null) {
			throw new BuildException(
					"No bean type to analyze defined." + " Please define value for attribute \"type\".");
		}
		...
		if (this.modelroot == null) {
			getProject().log("     [genrefinput]   no modelroot defined, force = true", Project.MSG_VERBOSE);
			force = true;
		}
	 ...
	 */
	protected static void generateConfLangDescr(final TypeRapidBean type, final Writer writer, final Project project) {
	 ...
	 * @param alreadydefined the map storing XML element / type combinations already
	 *                       described so far.
	 * @param writer         the writer
	 * @param project        the ant project
	 */
	private static void generateConfLangDescr(final TypeRapidBean type, final String xmlname,
		...
		try {
			...
			if (type.getDescription() != null) {
				writer.write("\t\t<description><![CDATA[" + type.getDescription() + " ]]></description>" + LF);
			} else {
			...
			/*
			 * int proptypesWithElementXmlBindingCount = 0; for (final TypeProperty proptype
			 * : type.getPropertyTypes()) { if (proptype.getXmlBindingType() ==
			 ...
			 * writer.write("EMPTY>"); } else { writer.write("(#PCDATA"); for (final
			 * TypeProperty proptype : type.getPropertyTypes()) { if
			 * (proptype.getXmlBindingType() == PropertyXmlBindingType.element) {
			 * writer.write(" | "); writer.write(proptype.getPropName()); } } for (final
			 * TypePropertyCollection colPropType : colProptypes) { if
			 * (colPropType.isComposition() && (!colPropType.isTransient())) { for (final
			 * String subelementname : getSubelementNames(colPropType, type)) {
			 * writer.write(" | "); writer.write(subelementname); } } } writer.write(")*>");
			 * }
			 */
			...
			for (final TypeProperty proptype : type.getPropertyTypes()) {
				if (proptype instanceof TypePropertyCollection && ((TypePropertyCollection) proptype).isComposition()) {
					for (final String subelementname : getSubelementNames((TypePropertyCollection) proptype, type)) {
				...
				for (final String subelementname : getSubelementNames(colPropType, type)) {
					...
					if (alreadydefined.get(subelementname) == null) {
						...
						if (alreadydefined.get(subelementname) instanceof TypeRapidBean) {
							throw new BuildException(
									"XML Element \"" + subelementname + "\" already used for bean type \""
											+ ((TypeRapidBean) alreadydefined.get(subelementname)).getName() + "\"");
						} else {
							throw new BuildException(
									"XML Element \"" + subelementname + "\" already used for property type \""
	...
	private static void writeAttribute(final TypeProperty proptype, final String propname, final Writer writer)
		...
		if (proptype.getDescription() != null) {
			writer.write("\t\t\t<description><![CDATA[" + proptype.getDescription() + "]]></description>" + LF);
		} else {
			writer.write("\t\t\t<description>@@@undefined@@@</description>" + LF);
		}
	...
	// */
	// private static int countAttributes(final TypeRapidBean type, final
	// List<TypePropertyCollection> colProptypes) {
	// // count attributes
	// int attrcount = 0;
	...
	// for (final TypeProperty beanproptype : beantype.getPropertyTypes()) {
	// if (beanproptype.getXmlBindingType() == PropertyXmlBindingType.element) {
	// writer.write(indent + "\tSubelement: " + beanproptype.getPropName() +
	// LF);
	// if (beanproptype.getDescription() != null) {
	// for (final String line :
	// StringHelper.split(beanproptype.getDescription(), "\n")) {
	// writer.write(indent + "\t\t" + line + LF);
	// }
	// }
	// }
	...
	// for (final TypePropertyCollection colPropType : colProptypes) {
	// if (colPropType.isComposition() && (!colPropType.isTransient())) {
	// for (final String subelementname : getSubelementNames(colPropType,
	// beantype)) {
	...
	// final TypeRapidEnum enumtype = ((TypePropertyChoice)
	// beanproptype).getEnumType();
	// if (enumtype.getDescription() != null) {
	...
	// for (final String line :
	// StringHelper.split(enumtype.getDescription(enumElem.name()), "\n")) {
	// writer.write(indent + "\t\t\t" + line + LF);
	// }
	// }
	// }
	...
	/**
	 * iterates recursively over a complete directory tree and determines the newest
	 * (latest) modification date of all the directories contained in the file
	 * system hierarchy.
	 */
