...
public class ConfigFileXmlTest {
	...
	public static void tearDownClass() {
		...
		File file1 = new File("renv_" + PlatformHelper.username()
				+ "_" + PlatformHelper.hostname()
				+ ".properties");
		file1.delete();
		new File("renv_" + PlatformHelper.username()
				+ "_" + PlatformHelper.hostname()
				+ ".cmd").delete();
	}
	...
	public void testReadConfiguration() {
		...
		ConfigFileXml file =
				(ConfigFileXml) unit.getConfigurations().get(0);
		Assert.assertNotNull(file.getTasks());
		final ConfigFileXmlTaskSetnodevalue task0 =
				(ConfigFileXmlTaskSetnodevalue) file.getTasks().get(0);
	...
	public void testConfigSetnodevalueCheckConfigRequired() throws FileNotFoundException {
		...
		try {
			...
			Assert.assertEquals("false", editor.retrieveNode("//settings/proxies/proxy/active").getFirstChild().getNodeValue());
			Assert.assertEquals("myproto", editor.retrieveNode("//settings/proxies/proxy/protocol").getFirstChild().getNodeValue());
			Assert.assertEquals("myserver1", editor.retrieveNode("//settings/servers/server[1]/id").getFirstChild().getNodeValue());
			Assert.assertEquals("yyy", editor.retrieveNode("//settings/testnode02/@attr1").getNodeValue());
			Assert.assertEquals("hello1", editor.retrieveNode("//settings/localRepository/@testattr").getNodeValue());
			Assert.assertEquals("hello2", editor.retrieveNode("//settings/localRepository/@testattr2").getNodeValue());
			Assert.assertEquals("hello first", editor.retrieveNode("//settings/emptytestnode01/firstnewsubelement").getFirstChild().getNodeValue());
			Assert.assertEquals("hello attr", editor.retrieveNode("//settings/emptytestnode01/firstnewsubelement/@test1").getFirstChild().getNodeValue());
			// TODO RENV insert a new XML element /attribute (optionally before or after another specified XML element / attr) with a set of identifying attributes (=> do not insert if it is already there)
			Assert.assertNotNull(editor.retrieveNode("//settings/testnode01"));
			Assert.assertNull(editor.retrieveNode("//settings/testnode01/subnodetodelete"));
			Assert.assertNull(editor.retrieveNode("//settings/testnode01/@attr1"));
		} finally {
	...
	public void testConfigSetnodevalueCreateNodeWithId01() {
		...
		try {
			...
			final ConfigFileEditorXml editor =
					new ConfigFileEditorXml(configFile, fileToConfigure);
