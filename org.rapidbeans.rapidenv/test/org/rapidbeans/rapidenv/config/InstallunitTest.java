...
public class InstallunitTest {
	...
	public void testCheckSemanticsNoSourceURL() {
		...
		try {
			...
			Assert.assertTrue(e.getMessage().startsWith("No source URL defined for installunit"));
			throw e;
		}
	}
	...
	public void testSubunits() {
		...
		Project project = (Project) doc.getRoot();
		Installunit test01 = project.findInstallunitConfiguration("test01");
		Assert.assertEquals("org.rapidenv.test01", test01.getFullyQualifiedName());
		Assert.assertEquals(new File("testdata/testtargetdir/org/rapidenv/test01/1.0.0").getAbsolutePath(), test01
		        .getHomedirAsFile().getPath());
		...
		Assert.assertEquals("org.rapidenv.test01/test2/testx", test01_2_x.getFullyQualifiedName());
		Installunit test01_2_1 = project.findInstallunitConfiguration("org.rapidenv.test01/test2/test1");
		Assert.assertEquals("org.rapidenv.test01/test2/test1", test01_2_1.getFullyQualifiedName());
	}
	...
	public void testSwapUnits1() {
		...
		Installunit.swapUnits(units);
		Assert.assertEquals("bluemel.unit1", units.get(0).getFullyQualifiedName());
	}
	...
	public void testSwapUnits3() {
		...
		Installunit.swapUnits(units);
		Assert.assertEquals("bluemel.unit3", units.get(0).getFullyQualifiedName());
		Assert.assertEquals("bluemel.unit2", units.get(1).getFullyQualifiedName());
		Assert.assertEquals("bluemel.unit1", units.get(2).getFullyQualifiedName());
	}
	...
	public void testSwapUnits4() {
		...
		Installunit.swapUnits(units);
		Assert.assertEquals("bluemel.unit4", units.get(0).getFullyQualifiedName());
		Assert.assertEquals("bluemel.unit3", units.get(1).getFullyQualifiedName());
		Assert.assertEquals("bluemel.unit2", units.get(2).getFullyQualifiedName());
		Assert.assertEquals("bluemel.unit1", units.get(3).getFullyQualifiedName());
	}
	...
	public void testAmbigoousSubunit() {
		...
		try {
			...
			Assert.assertTrue("Unexpected message text start: " + e.getMessage(),
	...
	public void testGetHomedir() {
		...
		Project project = (Project) doc.getRoot();
		Installunit ant = project.findInstallunitConfiguration("ant");
		Assert.assertEquals("testdata/testinstall/org/apache/ant/1.8.0", ant.getHomedir());
		Installunit antXalanSerializer = project.findInstallunitConfiguration("org.apache.ant/xalan.serializer");
		Assert.assertEquals("testdata/testinstall/org/apache/ant/1.8.0/lib", antXalanSerializer.getHomedir());
	}
	...
	public void testGetHomedirDeep() {
		...
		Assert.assertEquals("testdata/testtargetdir/org/rapidenv/test01/1.0.0", test01.getHomedir());
		Assert.assertEquals(new File("testdata/testtargetdir/org/rapidenv/test01/1.0.0").getAbsolutePath(), test01
		        .getHomedirAsFile().getAbsolutePath());
		Installunit test01_1 = project.findInstallunitConfiguration("org.rapidenv.test01/test1");
		Assert.assertEquals("testdata/testtargetdir/org/rapidenv/test01/1.0.0/test1/lib", test01_1.getHomedir());
		Installunit test01_2_x = project.findInstallunitConfiguration("org.rapidenv.test01/test2/testx");
		Assert.assertEquals("testdata/testtargetdir/org/rapidenv/test01/1.0.0/test2/testx", test01_2_x.getHomedir());
	}
}
