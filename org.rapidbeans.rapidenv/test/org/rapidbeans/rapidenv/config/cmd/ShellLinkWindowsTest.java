...
public class ShellLinkWindowsTest {
    ...
    public void testCreateProgrammatically() {
        ...
        assertEquals("echo Hello \"My Folks\"!& pause",
                shellLinkRead.getArguments().get(1).getValue());
    ...
    public void testReadWindows() {
        ...
        ShellLinkWindows shellLink = new ShellLinkWindows(
                new File("testdata/shelllink/testRead.lnk"));
        shellLink.load();
        ...
        assertEquals(new File(System.getenv("SYSTEMROOT")
                ...
                + "cmd.exe"), shellLink.getTargetPath());
        assertEquals(
                System.getenv("SYSTEMROOT") + File.separator + "system32",
                shellLink.getWorkingDirectory().getAbsolutePath());
        assertEquals(2,
                shellLink.getArguments().size());
        assertEquals("/C",
                shellLink.getArguments().get(0).getValue());
        assertFalse(shellLink.getArguments().get(0).getQuoted());
        assertEquals("echo Hello shortcut!& pause",
                shellLink.getArguments().get(1).getValue());
        assertTrue(shellLink.getArguments().get(1).getQuoted());
        assertTrue(
                shellLink.getIconFile().getAbsolutePath().endsWith(
                		"testdata" + File.separator
                		+ "shelllink" + File.separator
                		+ "test.ico"));
