...
//    		cmd.addArgument(new Argument("read x && echo \"x = \\\"${x}\\\"\""));
//    		cmd.setInput("XXX\n\n\n");
//    		result = cmd.execute();
...
//    		cmd.setSilent(true);
//    		cmd.addArgument(new Argument("/C"));
//    		cmd.addArgument(new Argument("FOR /F \"tokens=*\" %%A IN ('TYPE CON') DO SET INPUT=%%A && echo x = \"%INPUT%\""));
//    		cmd.setInput("XXX\n\n\n");
//    		result = cmd.execute();
//    		Assert.assertEquals("x = \"XXX\"\n", result.getStdout());
//    		break;
    ...
    public void testReadConfiguration() {
        RapidEnvInterpreter interpreter = new RapidEnvInterpreter(
                new CmdRenv(new String[] {
                        "-env", "testdata/env/envSystemCommand01.xml", "s"}));
        ...
        case linux:
            ...
            break;
        default:
            Assert.fail("Operating system \"" + PlatformHelper.getOs().name()
    ...
    /**
     * Test passing arguments to a Windows script interpreted by cmd.exe.
     * The SystemCommand is built by the program.
     *
     * Please note the limitation that cmd.exe will keep surrounding
     * quotes in arguments.
     * The script called in this test testArgPassing.cmd
     * demonstrates how to strip them afterwards.
     */
    @Test
    ...
    /**
     * Test passing arguments to a Windows script interpreted by cmd.exe.
     * The SystemCommand is read from an environment configuration file.
     *
     * Please note the limitation that cmd.exe will keep surrounding
     * quotes in arguments.
     * The script called in this test testArgPassing.cmd
    ...
    /**
     * Test passing arguments to a Windows VBS script interpreted by the
     * Windows Scripting Host (cscript.exe).
     * The SystemCommand is built by the programm
     *
     * Please note the limitation that cscript.exe ist not capable to receive
     * quotes in arguments.
     * The VBS script called in this test testArgPassing.vbs
     * demonstrates how to overcome this problem.
     */
    @Test
    public void testArgumentsPassingWindowsCscriptProg() {
        ...
        case windows:
            ...
            cmd.addArgument(new Argument(new File("testdata\\scripts\\windows\\testArgPassing.vbs").getAbsolutePath(), false));
