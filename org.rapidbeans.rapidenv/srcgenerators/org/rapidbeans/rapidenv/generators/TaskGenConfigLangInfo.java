...
public final class TaskGenConfigLangInfo extends Task {
	...
	private static void generateConfLangDescr(final TypeRapidBean type, final String xmlname,
		...
		try {
			...
			/*
			 * int proptypesWithElementXmlBindingCount = 0; for (final
			 * TypeProperty proptype : type.getPropertyTypes()) { if
			 * (proptype.getXmlBindingType() == PropertyXmlBindingType.element)
			 * { proptypesWithElementXmlBindingCount++; } } if
			 * (colProptypes.size() == 0 && proptypesWithElementXmlBindingCount
			 * == 0) { writer.write("EMPTY>"); } else {
			 * writer.write("(#PCDATA"); for (final TypeProperty proptype :
			 * type.getPropertyTypes()) { if (proptype.getXmlBindingType() ==
			 * PropertyXmlBindingType.element) { writer.write(" | ");
			 * writer.write(proptype.getPropName()); } } for (final
			 * TypePropertyCollection colPropType : colProptypes) { if
			 * (colPropType.isComposition() && (!colPropType.isTransient())) {
			 * for (final String subelementname :
			 * getSubelementNames(colPropType, type)) { writer.write(" | ");
			 * writer.write(subelementname); } } } writer.write(")*>"); }
			 */
