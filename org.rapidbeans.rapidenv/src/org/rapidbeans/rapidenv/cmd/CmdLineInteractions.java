...
public class CmdLineInteractions {
     ...
     */
    public static String enterValue(final InputStream in, final PrintStream out,
            final Property prop, final String defaultValue) {
                        ...
                        if (newValue == null || StringHelper.strip(newValue, StringHelper.StripMode.both).length() == 0) {
                            ...
                            if (prop.getCreateIfNotExist()) {
                                if (!new File(newValue).exists()) {
                                    if (promptYesNo(in, out, prop.getFiletype().name()
                                            + " " + newValue + " does not exist. Create it?", true)) {
                            ...
                            if (!new File(newValue).exists()) {
                                out.println("  Invalid input: " + prop.getFiletype().name()
        ...
        case url:
            ...
            while (!ok) {
                ...
                try {
                   ...
                   out.println("  Invalid input: URL "
                           + newValue + " is not valid.");
                  ok = false;
               }
            }
            break;
     ...
     * @return the value entered or the default value if nothing is entered
     */
    protected static String enterValue(
     ...
     */
    public static boolean promptYesNo(final InputStream in,
            final PrintStream out,
    ...
    protected static String read(final InputStream sin) {
        String line = null;
        try {
            java.io.BufferedReader rdin = new java.io.BufferedReader(
