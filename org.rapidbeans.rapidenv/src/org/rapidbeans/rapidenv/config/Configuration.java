...
public abstract class Configuration extends RapidBeanBaseConfiguration {
    ...
    /**
     * Hack to overcome current RapidBeans framework deficiency.
     * Remove when it is fixed in RapidBeans.
     */
    	...
    	if (getCleanupfilesonconfig() != null) {
    		for (String filename : StringHelper.split(getCleanupfilesonconfig(), ",")) {
    			...
    			if (file.exists()) {
    				if (file.delete()) {
    					if (interpreter != null) {
    						interpreter.getOut().println("    Cleaned up file \""
    								+ file.getAbsolutePath() + "\"");
    					}
    				} else {
    					throw new RapidEnvException("Could not delete file \""
    							+ file.getAbsolutePath() + "\".");
    				}
    			} else {
					if (interpreter != null) {
						interpreter.getOut().println("    Nothing to do for cleaup of file \""
								+ file.getAbsolutePath() + "\"");
					}
    			}
    		}
