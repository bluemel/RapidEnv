...
public class ShellLinkIcon extends RapidBeanBaseShellLinkIcon {
    ...
    private boolean checkShellLinkWindows(final boolean execute, final String title,
            final File shellLinkFolder, final File executeInFolder, final File executable,
            final List<Argument> args) {
        boolean ok = true;
        final List<Argument> arguments = new ArrayList<Argument>();
        for (final Argument arg : args) {
            final Argument argument = new Argument(
                    this.interpret(arg.getValue()));
        ...
        if (ok) {
            for (int i = 0; ok && i < newArgumentsCount; i++) {
                ...
                if (!newArg.equals(oldArg)) {
                    ...
                    msgcheck = "shell link icon \"" + title + "\" needs to be updated" + " in folder \""
                            + shellLinkFolder.getAbsolutePath() + "\".\n"
                            + "    Argument number " + Integer.toString(i + 1) + " has changed\n"
                            + "      from \"" + oldArg + "\"\n"
                            + "        to \"" + newArg + "\"";
                }
                if (ok) {
                    if (arguments.get(i).getQuoted() != shellLink.getArguments().get(i).getQuoted()) {
                        ...
                        msgcheck = "shell link icon \"" + title + "\" needs to be updated" + " in folder \""
                                + shellLinkFolder.getAbsolutePath() + "\".\n"
                                + "    Argument number " + Integer.toString(i + 1) + " has changed it's quoting.\n";
                    }
                }
            }
        }
        ...
        if (ok) {
            ...
            if (!newIconFilePath.equals(oldIconFilePath)) {
                ok = false;
                msgcheck = "shell link icon \"" + title + "\" needs to be updated"
                        + " in folder \"" + shellLinkFolder.getAbsolutePath() + "\".\n"
                        + "    Icon file has changed\n"
                        + "    from \"" + oldIconFilePath + "\"\n"
                        + "      to \"" + newIconFilePath + "\"";
            }
        }
        if (ok) {
    ...
    private boolean checkShellLinkLinux(final boolean execute, final String title,
            final File shellLinkFolder, final File executeInFolder, final File executable,
            final List<Argument> arguments) {
        boolean ok = true;
        final File shellLinkFile = new File(shellLinkFolder, title + ".desktop");
        final ConfigFileEditorProperties shellLink =
                new ConfigFileEditorProperties(new ConfigFileProperties(), shellLinkFile);
        final File iconFile = new File(interpret(getIconfile()));
        final RapidEnvInterpreter interpreter = RapidEnvInterpreter.getInstance();
        ...
        if (ok) {
            if (!title.equals(shellLink.getProperty("Name"))) {
                ...
                msgcheck = "shell link icon \"" + title + "\" needs to be updated" + " in folder \""
                        + shellLinkFolder.getAbsolutePath() + "\".\n" + "    Title has changed\n"
                        + "      from \"" + shellLink.getProperty("Name") + "\"\n" + "        to \""
                        + title + "\"";
            }
        }
        if (ok) {
        ...
        if (execute) {
            if (ok) {
                ...
                try {
                    ...
                    shellLink.setProperty("[Desktop Entry]", "Name",
                            title);
