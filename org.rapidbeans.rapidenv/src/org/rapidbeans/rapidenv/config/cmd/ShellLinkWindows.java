...
public class ShellLinkWindows {
    ...
    /**
     * The shortcut's command working directory.
     * Must exist and be a directory / folder.
     */
    ...
    /**
     * The shortcut's hot key.
     * A combination of Keyboard key's.
     */
     ...
     * Constructor.
     *
     * @param shorcutFile the shortcut file.
     *     Usually ends with ".lnk".
     */
    ...
    public void save() {
        ...
        if (this.iconFile == null) {
            iconLocation = "%SystemRoot%"
                    + File.separator + "system32"
                    + File.separator + "csript.exe"
                    + ",0";
        } else {
        ...
        command.addArgument(new Argument(
                Integer.toString(this.windowStyle.ordinal() + 1)));
    ...
    private String keyCode2Name(final int keyCode) {
        String keyName = null;
        switch (keyCode) {
        case KeyEvent.VK_CONTROL: keyName = "Ctrl"; break;
        case KeyEvent.VK_ALT: keyName = "Alt"; break;
        case KeyEvent.VK_ALT_GRAPH: keyName = "AltGraph"; break;
        case KeyEvent.VK_SHIFT: keyName = "Shift"; break;
        default:
    ...
    public void load() {
        ...
        try {
            props.load(new StringReader(StringHelper.escape(
                    result.getStdout(),
                    new EscapeMap(new String[]{"\\", "\\\\"}))));
        } catch (IOException e) {
            throw new RapidEnvException("IO exception while reading standard out: "
                    + e.getMessage(), e);
        }
