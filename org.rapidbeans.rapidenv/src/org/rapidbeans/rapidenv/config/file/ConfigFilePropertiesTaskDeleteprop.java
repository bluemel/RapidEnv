 ...
 */
public class ConfigFilePropertiesTaskDeleteprop extends RapidBeanBaseConfigFilePropertiesTaskSetpropvalue {
	...
	public boolean check(final boolean execute, final boolean silent) {
		...
		if (propval == null) {
			if (execute) {
				...
				msg += "\"" + getName() + "\"" + " with value \"" + getValue() + "\"" + " in file "
				...
				msg += "\"" + getName() + "\"" + " with value \"" + getValue() + "\"" + " not found in file "
				        + fileCfg.getPathAsFile().getAbsolutePath();
				RapidEnvInterpreter.log(Level.FINE, msg);
			...
			if (propval.equals(getValue())) {
				...
				msg += "\"" + getName() + "\"" + " already has value \"" + getValue() + "\"" + " in file "
				        + fileCfg.getPathAsFile().getAbsolutePath();
				RapidEnvInterpreter.log(Level.FINE, msg);
			} else {
				if (execute) {
					...
					msg += "\"" + getName() + "\"" + " to \"" + getValue() + "\"" + " in file "
					        + fileCfg.getPathAsFile().getAbsolutePath();
					if (!silent) {
						interpreter.getOut().println(msg);
					}
	...
	public String getCommenttag() {
		if (super.getCommenttag() != null && super.getCommenttag().length() == 0) {
