...
public abstract class ConfigFile extends RapidBeanBaseConfigFile {
     ...
     *
     * @param execute
     *     if false only execute the check if the configuration is necessary
    ...
    public boolean check(final boolean execute) {
        ...
        if (getSourceurl() != null) {
            if (url.getProtocol().equals("file")) {
                ...
                if (!targetfile.exists()) {
                    if (execute) {
                        ...
                        final String msg = "    copied " + sourcefile.getAbsolutePath()
                        + " to file " + targetfile.getAbsolutePath();
                        interpreter.getOut().println(msg);
                        configured = true;
                    } else {
                        final String msg = "File to configure \""
            ...
            if (!targetfile.exists()) {
                if (execute) {
                    ...
                    final String msg = "    created new configuration file "
            ...
            case sourcenewer:
                if (sourcefile.lastModified() > targetfile.lastModified()) {
                    if (execute) {
                        FileHelper.copyFile(sourcefile, targetfile);
                        final String msg = "    copied " + sourcefile.getAbsolutePath()
                                + " over outdated file " + targetfile.getAbsolutePath();
                        RapidEnvInterpreter.log(Level.FINE, msg);
                        interpreter.getOut().println(msg);
                        configured = true;
                    } else {
                ...
                if (this.getTasks() != null && this.getTasks().size() > 0) {
                    ...
                    for (final ConfigurationTask cfgTask : this.getTasks()) {
                        if (!cfgTask.check(true, true)) {
                            throw new RapidEnvException("Problem to apply configuration "
                                    + cfgTask.toString());
                        }
                    }
                    this.editor.save();
                    ...
                    if (!tmpfile.delete()) {
                        throw new RapidEnvException("Problems while trying to delete "
        ...
        if (getCanread() && (!targetfile.canRead())) {
            if (execute) {
                interpreter.getOut().println("Add read rights to "
                        + "file to configure \""
                        + targetfile.getAbsolutePath()
                        + "\".");
        ...
        if (getCanwrite() && (!targetfile.canWrite())) {
            if (execute) {
                interpreter.getOut().println("Add write rights to "
                        + "file to configure \""
                        + targetfile.getAbsolutePath()
                        + "\".");
        ...
        if (getCanexecute() && (!targetfile.canExecute())) {
            if (execute) {
                interpreter.getOut().println("Add execution rights to "
                        + "file to configure \""
                        + targetfile.getAbsolutePath()
                        + "\".");
        ...
        if (!getCanread() && targetfile.canRead()) {
            if (execute) {
                interpreter.getOut().println("Remove read rights from "
                        + "file to configure \""
                        + targetfile.getAbsolutePath()
                        + "\".");
        ...
        if (!getCanwrite() && targetfile.canWrite()) {
            if (execute) {
                interpreter.getOut().println("Remove write rights from "
                        + "file to configure \""
                        + targetfile.getAbsolutePath()
                        + "\".");
                if (!targetfile.setWritable(false)) {
                    throw new RapidEnvException("Withdrawing write rights"
                            + " from configuration file \""
                            + targetfile.getAbsolutePath()
                            + "\" failed.");
                } else {
        ...
        if (PlatformHelper.getOs() != OperatingSystem.windows) {
            if (!getCanexecute() && targetfile.canExecute()) {
                if (execute) {
                    interpreter.getOut().println("Remove execution rights from "
                            + "file to configure \""
                            + targetfile.getAbsolutePath()
                            + "\".");
                    if (!targetfile.setExecutable(false)) {
                        throw new RapidEnvException("Withdrawing execution rights"
                                + " from configuration file \""
                                + targetfile.getAbsolutePath()
                                + "\" failed.");
                    } else {
    ...
    public void createNewFile(final File targetfile) throws IOException {
        if (!targetfile.createNewFile()) {
            throw new RapidBeansRuntimeException("Could not create new file \""
                    + targetfile.getAbsolutePath() + "\"");
        }
	}
	...
	public String getPath() {
    	...
    	final RapidEnvInterpreter interpreter = RapidEnvInterpreter.getInstance();
    	if (interpreter != null) {
    		path = interpreter.interpret(
    				getParentInstallunit(), getParentPropertyRenv(), path);
    	}
    	return path;
    }
    ...
    public String getNewlineChars() {
    	...
    	default:
    		throw new RapidEnvException(
    				"Can not interpret " + getNewline().name());
    	}
    	return newlineChars;
    }
    ...
    protected URL getSourceurlAsUrl() {
        ...
        try {
            ...
            throw new RapidEnvConfigurationException(
                    ...
                    + " in file configuration with sourceurl \""
                    + getSourceurl() + "\"", e);
        }
    }
