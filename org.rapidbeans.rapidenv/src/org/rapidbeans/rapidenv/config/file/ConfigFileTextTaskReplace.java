...
public class ConfigFileTextTaskReplace extends RapidBeanBaseConfigFileTextTaskReplace {
    ...
    public boolean check(final boolean execute, final boolean silent) {
        ...
        if (matches.length > 0) {
            if (execute) {
                final String replaceby = interpreter.interpret(
                        (Installunit) getParentBean().getParentBean(),
                        null, getReplaceby());
                editor.replace(getRegexp(), replaceby);
                String msg1;
                if (matches.length == 1) {
                    msg1 = "Replaced 1 ocurrence of \""
                            + getRegexp() + "\"\n    by \"" + replaceby
                            + "\"\n    in file " + fileCfg.getPathAsFile().getAbsolutePath();
                } else {
                    msg1 = "Replaced " + Integer.toString(matches.length)
                            + " ocurrences of \"" + getRegexp()
                            + "\"\n    by \"" + replaceby + "\"\n    in file "
            ...
            final String msg3 = "no matches to replace found"
                    + " with regular expression: \"" + getRegexp() + "\".";
            if (execute) {
