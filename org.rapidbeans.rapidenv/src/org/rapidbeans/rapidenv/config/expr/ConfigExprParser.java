...
public final class ConfigExprParser {
        ...
        for (int i = 0; i < len; i++) {
            ...
            case basic:
                ...
                case '$':
                    ...
                    if (text.length() > 0) {
                        expr.addChild(new ConfigExprStringLiteral(
                                enclosingUnit, enclosingProp, text.toString()));
                        text.setLength(0);
                    }
                    state = State.envvar;
                    break;
                case '(':
                    if (text.length() > 0) {
                        ...
                        if (splitResult.getFuncname() == null
                                || splitResult.getFuncname().length() == 0) {
                            throw new RapidEnvException(
                                    "parsed '(' without preceeding function name");
                        }
                        if ((splitResult.getBefore() != null)
                                && splitResult.getBefore().length() > 0) {
                            expr.addChild(new ConfigExprStringLiteral(
                                    enclosingUnit, enclosingProp,
                                    splitResult.getBefore()));
                        }
                        text.setLength(0);
                    } else {
                        throw new RapidEnvException(
            ...
            case envvar:
                switch (c) {
                case '{':
                    final int iEnd = countToClosingBrace('{', '}',
                            expressionString, i + 1);
                    final ConfigExpr subexpression = new ConfigExprTopLevel(
                            enclosingUnit, enclosingProp, expressionString.substring(i + 1, i + iEnd),
                            		escapeLiterals);
                    final ConfigExprProperty propExpression = new ConfigExprProperty(
                            enclosingUnit, enclosingProp, subexpression, escapeLiterals);
                ...
                default:
                    ...
                    throw new AssertionError("Unexpected character '"
                            + c + "' in expression \"" + expressionString
                            + "\" at position " + i);
                }
                break;
                        ...
                        try {
                            ...
                            if (e.getCause() != null
                                ...
                                final ConfigExprStringLiteral literalExpr = new ConfigExprStringLiteral(
                                            expr.getEnclosingInstallUnit(), expr.getEnclosingProperty(), literalPushback.toString());
                                ...
                                final String message = "No interpreter class found for function \""
                                        + funcname + "()\".";
            ...
            case literal:
                ...
                case '\'':
                    if (literal.length() > 0) {
                        expr.addChild(new ConfigExprStringLiteral(enclosingUnit, enclosingProp,
                                literal.toString()));
                        literal.setLength(0);
                    }
                    state = State.basic;
    ...
    private static int countToClosingBrace(
            final char openingBrace, final char closingBrace,
