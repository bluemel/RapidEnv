...
public class AntTaskTest {
	...
	public static void setUpClass() {
		...
		// }
		// FileHelper.copyFile(new File("target/generated-dtds/env.dtd"), new
		// File("../../env.dtd"));
	...
	public void testExecuteCopy() throws IOException {
		try {
			Assert.assertFalse(new File("src/test/resources/unpack/artifacts1.xml").exists());
			...
			arg1.setName("file");
			arg1.setValue(new File("src/test/resources/unpack/artifacts.xml").getAbsolutePath());
			arg1.setValuetype(Argtype.file);
			task.addArgument(arg1);
			Argument arg2 = new Argument();
			arg2.setName("tofile");
			arg2.setValue(new File("src/test/resources/unpack/artifacts1.xml").getAbsolutePath());
			arg2.setValuetype(Argtype.file);
			task.addArgument(arg2);

			task.execute();

			Assert.assertTrue(new File("src/test/resources/unpack/artifacts1.xml").exists());
			RapidEnvTestHelper.assertFilesEqual(new File("src/test/resources/unpack/artifacts.xml"),
					new File("src/test/resources/unpack/artifacts1.xml"));
		} finally {
			if (new File("src/test/resources/unpack/artifacts1.xml").exists()) {
				Assert.assertTrue(new File("src/test/resources/unpack/artifacts1.xml").delete());
			}
		}
	}
}
