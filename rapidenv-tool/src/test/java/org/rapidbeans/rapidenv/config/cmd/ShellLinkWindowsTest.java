...
public class ShellLinkWindowsTest {
	...
	/**
	 * Test method for
	 * {@link org.rapidbeans.rapidenv.config.cmd.ShellLinkWindows#save()}.
	 */
	@Test
	public void testCreateProgrammatically() {
		...
		File linkFile = new File("src/test/resources/shelllink/testCreate.lnk");
		if (linkFile.exists()) {
		...
		shellLink.setTargetPath(
				new File(System.getenv("SYSTEMROOT") + File.separator + "System32" + File.separator + "cmd.exe"));
		shellLink.setWorkingDirectory(new File(System.getenv("SYSTEMROOT") + File.separator + "System32"));
		shellLink.addArgument(new Argument("/C"));
		shellLink.addArgument(new Argument("echo Hello \"My Folks\"!& pause", true));
		shellLink.setIconFile(new File("src/test/resources/shelllink/test.ico"));
		...
		shellLinkRead.load();
		assertEquals("This is a second \"test shortcut\".", shellLinkRead.getDescription());
		assertEquals(new File(System.getenv("SYSTEMROOT") + File.separator + "System32"),
				shellLinkRead.getWorkingDirectory());
		...
		assertEquals("echo Hello \"My Folks\"!& pause", shellLinkRead.getArguments().get(1).getValue());
		assertEquals(true, args.get(1).getQuoted());
		assertEquals(new File("src/test/resources/shelllink/test.ico").getAbsolutePath(),
				shellLinkRead.getIconFile().getAbsolutePath());
		assertEquals(3, shellLinkRead.getIconNumber());
		assertEquals(KeyEvent.VK_ALT, shellLinkRead.getHotKey().get(0).intValue());
		assertEquals(KeyEvent.VK_SHIFT, shellLinkRead.getHotKey().get(1).intValue());
	...
	/**
	 * Test method for
	 * {@link org.rapidbeans.rapidenv.config.cmd.ShellLinkWindows#load()}.
	 */
	@Test
	public void testReadWindows() {
		...
		ShellLinkWindows shellLink = new ShellLinkWindows(new File("src/test/resources/shelllink/testRead.lnk"));
		shellLink.load();
		assertEquals("This is a test shortcut for reading shortcuts.", shellLink.getDescription());
		...
		assertEquals("echo Hello shortcut!& pause", shellLink.getArguments().get(1).getValue());
		assertTrue(shellLink.getArguments().get(1).getQuoted());
		assertTrue(shellLink.getIconFile().getAbsolutePath().endsWith("src" + File.separator + "test" + File.separator
				+ "resources" + File.separator + "shelllink" + File.separator + "test.ico"));
		assertEquals(0, shellLink.getIconNumber());
		assertSame(ShortcutWindowStyle.maximizedFocus, shellLink.getWindowStyle());
		assertEquals(3, shellLink.getHotKey().size());
		assertEquals(new Integer(KeyEvent.VK_ALT), shellLink.getHotKey().get(0));
		assertEquals(new Integer(KeyEvent.VK_CONTROL), shellLink.getHotKey().get(1));
