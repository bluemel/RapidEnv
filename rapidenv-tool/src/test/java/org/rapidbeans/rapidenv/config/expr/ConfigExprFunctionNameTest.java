/*
 * RapidEnv: ConfigExprFunctionNameTest.java
 *
...
public class ConfigExprFunctionNameTest {
	...
	public void testInterpret() {
		...
		ConfigExprTopLevel expr = new ConfigExprTopLevel(new Installunit(new String[] { "", "", "jdk" }), null,
				"homedir()", false);
		assertEquals(new File("target/testinstall/jdk/1.6.0").getAbsolutePath(), expr.interpret());
		expr = new ConfigExprTopLevel(new Installunit(new String[] { "", "org.apache", "ant" }), null, "name()", false);
		assertEquals("ant", expr.interpret());
	}
}
