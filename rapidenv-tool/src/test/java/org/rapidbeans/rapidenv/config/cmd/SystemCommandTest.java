...
public class SystemCommandTest {
	...
	public void testProcessInput() throws IOException, InterruptedException {
		...
		case windows:
			...
			// cmd.addArgument(new Argument("/C"));
			// cmd.addArgument(new
			// Argument("FOR /F \"tokens=*\" %%A IN ('TYPE CON') DO SET INPUT=%%A && echo x
			// = \"%INPUT%\""));
			// cmd.setInput("XXX\n\n\n");
			// result = cmd.execute();
			// Assert.assertEquals("x = \"XXX\"\n", result.getStdout());
	...
	public void testReadConfiguration() {
		RapidEnvInterpreter interpreter = new RapidEnvInterpreter(
				new CmdRenv(new String[] { "-env", "src/test/resources/env/envSystemCommand01.xml", "s" }));
	 ...
	 * arguments. The script called in this test testArgPassing.cmd demonstrates how
	 * to strip them afterwards.
	 */
	@Test
	public void testArgumentsPassingWindowsCmdProg() {
		...
		case windows:
			...
			cmd.addArgument(new Argument("src\\test\\resources\\scripts\\windows\\testArgPassing.cmd", false));
			...
			CommandExecutionResult result = cmd.execute();
			Assert.assertEquals(
					String.format("Command excection failed:\n%s\n@@@ stdout @@@@@\n%s@@@ stderr @@@@@@\n%s@@@@@@@@@",
							cmd.getCommandline(), result.getStdout(), result.getStderr()),
	...
	public void testArgumentsPassingWindowsCmdConf() {
		...
		case windows:
			...
			Assert.assertEquals(String.format(
					"Command execution failed unexpectedly, cmd:\n%s\n@@@ stdout\n%s@@@ stderr\n%s@@@@@@@@@@@@@", //
					cmd.getCommandline(), result.getStdout(), result.getStderr()), 0, result.getReturncode());
			Assert.assertEquals("Test Argument passing:\n" + "argument 1: @xxx@\n" + "argument 2: @\"a b c\"@\n"
					+ "argument 3: @\"\\\"x\\\" \\\"y\\\" \\\"z\\\"\"@\n" + "argument 2': @a b c@\n"
					+ "argument 3': @\"x\" \"y\" \"z\"@\n", result.getStdout());
			Assert.assertEquals("", result.getStderr());
	 ...
	 *
	 * Please note the limitation that cscript.exe ist not capable to receive quotes
	 * in arguments. The VBS script called in this test testArgPassing.vbs
	...
	public void testArgumentsPassingWindowsCscriptProg() {
		...
		case windows:
			...
			cmd.addArgument(new Argument(
					new File("src\\test\\resources\\scripts\\windows\\testArgPassing.vbs").getAbsolutePath(), false));
