...
public class SystemCommand extends RapidBeanBaseSystemCommand {
		...
		if (exe != null) {
			...
			if (!file.canExecute()) {
				throw new RapidEnvException(
						"No execution rights" + " for executable file \"" + file.getAbsolutePath() + "\"");
			}
		}
		return file;
	}
	 ...
	 *
	 * @param execute if false only execute the check if the configuration is
				...
				for (final SystemCommand cmd : getVerifycmds()) {
					try {
						RapidEnvInterpreter.log(Level.FINER,
								"Executing verify command " + cmd.getClass().getName() + "::" + cmd.getCommandline());
						cmd.execute();
						RapidEnvInterpreter.log(Level.FINER,
								"verify command succeeded: " + cmd.getClass().getName() + "::" + cmd.getCommandline());
					} catch (RuntimeException e) {
						RapidEnvInterpreter.log(Level.FINER,
								"verify command failed: " + cmd.getClass().getName() + "::" + cmd.getCommandline());
						ok = false;
						break;
					}
				}
			...
			for (final CmdCondStdout stdCondition : getStdouts()) {
				...
				if (stdCondition.getContainsmatch() != null) {
					...
					RapidEnvInterpreter.log(Level.FINE,
							"Checking if stdout = \"" + out + "\" matches \"" + regexp + "\"");
	...
	private String[] getCommandArray() {
		...
		if (exeQuoted) {
			final EscapeMap map = new EscapeMap(
					new String[] { "\b", "\\b", "\n", "\\n", "\r", "\\r", "\t", "\\t", "\"", "\\\"", "\\", "\\\\" });
	...
	class CommandStreamReader extends Thread {
		...
		public void run() {
			try {
				...
				throw new RapidEnvException("Exception during execution of " + " system command \"" + cmdline + "\"",
						e);
			} finally {
				try {
					reader.close();
	...
	public File getExecutableAsFile() {
		return getExecutableAsFileStat(getExecutable(), null);
	}
