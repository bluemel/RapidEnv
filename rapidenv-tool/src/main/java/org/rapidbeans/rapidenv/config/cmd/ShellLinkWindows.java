...
public class ShellLinkWindows {
	 ...
	 * Constructor.
	 *
	 * @param shorcutFile the shortcut file. Usually ends with ".lnk".
	 */
	...
	public void save() {
		...
		final File devEnvScriptFile = new File("src/main/scripts/windows/shortcutWrite.vbs");
		File scriptFile = new File(
				System.getenv("RAPID_ENV_HOME") + File.separator + "bin" + File.separator + "shortcutWrite.vbs");
		...
		if (this.targetPath == null) {
			throw new RapidEnvConfigurationException(
					"No target path specified for windows icon \"" + this.file.getAbsolutePath() + "\"");
		}
		...
		if (this.workingDirectory == null) {
			throw new RapidEnvConfigurationException(
					"No working directory specified for windows icon \"" + this.file.getAbsolutePath() + "\"");
		}
		...
		if (result.getReturncode() != 0) {
			throw new RapidEnvCmdExecutionException(String.format(
					"Error during writing windows shortcut file \"%s\":\n" + "returncode = %d\n"
							+ "stdout @@@@@@@@@@@@\ns%sstderr @@@@@@@@@@@@@\n%s@@@@@@@@@@@@@@@@@\n"
							+ "commandline = \"%s\"\"",
					getFile().getAbsolutePath(), result.getReturncode(), result.getStdout(), result.getStderr(),
	...
	public void load() {
		...
		command.setExecutable("cscript.exe");
		File scriptFile = new File(
				System.getenv("RAPID_ENV_HOME") + File.separator + "bin" + File.separator + "shortcutRead.vbs");
		if (!scriptFile.exists()) {
			final File devEnvScriptFile = new File("src/main/scripts/windows/shortcutRead.vbs");
			if (devEnvScriptFile.exists()) {
				scriptFile = devEnvScriptFile;
			}
		}
		...
		if (result.getReturncode() != 0) {
			throw new RapidEnvCmdExecutionException(String.format(
					"Error during reading windows shortcut file \"%s\"\n"
							+ "returncode = %d\nstdout @@@@@@@@@@@@@@\n%sstderr @@@@@@@@@@@@\n&s",
					getFile().getAbsolutePath(), result.getReturncode(), result.getStdout(), result.getStderr()));
		}
		...
		try {
			props.load(new StringReader(
					StringHelper.escape(result.getStdout(), new EscapeMap(new String[] { "\\", "\\\\" }))));
		} catch (IOException e) {
	...
	/**
	 * @param description the shortcut's description to set
	 */
	...
	/**
	 * @param targetPath the shortcut's command target path
	 */
	...
	/**
	 * @param args the shortcut's command arguments.
	 */
	...
	/**
	 * @param workingDirectory the shortcut's working directory
	 */
	...
	/**
	 * @param iconFile the icon file to set
	 */
	...
	/**
	 * @param windowStyle the windowStyle to set
	 */
	...
	public static File getStartMenuFolder() {
		...
		RapidEnvInterpreter.log(Level.FINE,
				"start menu folder 2: " + startMenuFolder.getAbsolutePath() + ": " + startMenuFolder.exists());
